      ...
      mbProducer( ps.getParameter< InputTag >("mbProducer") )
{
    ...
    //RJ
    ptLUTset = ps.getParameter<edm::ParameterSet>("PTLUT");
...
void L1TCSCTF::bookHistograms(DQMStore::IBooker &ibooker, edm::Run const&, edm::EventSetup const&)
{
    ...
    //RJ
    ME11a_WG_Ptraw = ibooker.book2D("ME11a_WG_Ptraw", "ME11a_WG_Ptraw", 100,0,100, 112,0,112);
    ME11a_WG_Pt    = ibooker.book2D("ME11a_WG_Pt", "ME11a_WG_Pt", 100,0,100, 112,0,112);

    ME11b_WG_Ptraw = ibooker.book2D("ME11b_WG_Ptraw", "ME11b_WG_Ptraw", 100,0,100, 112,0,112);
    ME11b_WG_Pt    = ibooker.book2D("ME11b_WG_Pt", "ME11b_WG_Pt", 100,0,100, 112,0,112);
...
void L1TCSCTF::analyze(const Event& e, const EventSetup& c)
{
    ...
    if( c.get< L1MuTriggerScalesRcd > ().cacheIdentifier() != m_scalesCacheID ||
        ...
        //RJ
        ptLUTs_ = new CSCTFPtLUT(ptLUTset, ts, tpts);
    ...
    if( trackProducer.label() != "null" ) {
        ...
        for(L1CSCTrackCollection::const_iterator trk=tracks->begin(); trk<tracks->end(); trk++) {
            ...
            if (thePtAddress.track_fr) {
                csctf_trPtBit.push_back(thePtData.front_rank&0x1f);
            } else {
                csctf_trPtBit.push_back(thePtData.rear_rank&0x1f);
            }
            double csctf_trPt = (tpts->getPtScale()->getLowEdge(csctf_trPtBit.back()));
            ...
            for(CSCCorrelatedLCTDigiCollection::DigiRangeIterator trackStub=lctsOfTracks.begin(); trackStub!=lctsOfTracks.end(); trackStub++) {
                ...
                for(CSCCorrelatedLCTDigiCollection::const_iterator lct=range2.first; lct!=range2.second; lct++) {
                    ...
                    if(station==0 && ring==1) {
                        if(strip<4.*32) {
			    // ME11b
			    ME11b_WG_Ptraw -> Fill(csctf_trPt, keyWire);
                            if(keyWire<11) {
				std::cout << "ME11b --> strip: " << strip << " WG: " << keyWire << " Pt: " << csctf_trPt << std::endl;
				ME11b_WG_Pt -> Fill(csctf_trPt, keyWire);
			    }
                        } else {
			    // ME11a
			    ME11a_WG_Ptraw -> Fill(csctf_trPt, keyWire);
                            if(keyWire>14) {
				std::cout << "ME11a --> strip: " << strip << " WG: " << keyWire << " Pt: " << csctf_trPt << std::endl;
				ME11a_WG_Pt -> Fill(csctf_trPt, keyWire);
			    }
                        }
                    }
